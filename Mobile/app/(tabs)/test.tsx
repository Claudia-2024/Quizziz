...
const TestQuizPage: React.FC = () => {
  ...
  const [selectedEvalIndex, setSelectedEvalIndex] = useState<number | null>(
    null
  );
  ...
  const handleStartTest = async () => {
    try {
      ...
      const res = await api.post<any>(
        ENDPOINTS.evaluations.start(String(evaluationId)),
        startBody
      );
      ...
      const respIdRaw =
        ...
        res?.id ??
        res?.responseId ??
        res?.data?.responseSheetId ??
        res?.data?.responsheetId;
      ...
      const dur = Math.max(
        0,
        hmsToSeconds(evalItem?.endTime) - hmsToSeconds(evalItem?.startTime)
      );
  ...
  const saveCurrentAnswer = async () => {
    try {
      ...
      const payload = {
        answers: [
          { questionId: q.id, type: "MCQ", selectedOption: selectedOption },
        ],
      };
      await api.post(
        ENDPOINTS.evaluations.saveAnswers(String(responseSheetId)),
        payload
      );
      setSelections((prev) => ({
        ...prev,
        [q.id]: selectedOption ?? prev[q.id],
      }));
    } catch (e) {
  ...
  const submitEvaluation = async () => {
    try {
      ...
      for (const q of questions) {
        ...
        if (cid != null)
          all.push({ questionId: q.id, type: "MCQ", selectedOption: cid });
      }
      const payload = { answers: all };
      await api.post(
        ENDPOINTS.evaluations.submit(String(responseSheetId)),
        payload
      );
    } catch (e) {
  ...
  if (testStarted) {
    ...
    return (
      <View style={styles.container}>
        {/* Progress */}
        <Text style={styles.progress}>{`${currentQuestion + 1} of ${
          questions.length
        } `}</Text>
            ...
            {evaluations.length === 0 ? (
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => {
                  ...
                  const title = `${item.type || "Evaluation"} — ${[code, name]
                    .filter(Boolean)
                    .join(" — ")}`.trim();
      ...
      <Modal visible={showReadyModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            />
            <Text style={styles.modalText}>
              Your test has been uploaded click below to start!
            </Text>
            {startError ? (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {error ? (
              ...
              <Text style={styles.modalText}>
                No questions are available at this time.
              </Text>
            )}
            <TouchableOpacity
              style={styles.startButton}
